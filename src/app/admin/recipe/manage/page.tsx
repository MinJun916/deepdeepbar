...
const AdminRecipeManagePage = async ({ searchParams }: { searchParams: SearchParams }) => {
  ...
  const [{ data: recipesData, error: recipesError }, { data: menusData }, { data: glassTypesData }] =
    await Promise.all([
      supabase
        .schema('public')
        .from(recipeTable)
    ...
    <main className="min-h-screen bg-[radial-gradient(120%_90%_at_50%_0%,#fcf8f2_0%,#f3ece2_56%,#ece2d6_100%)] text-[#1f2937]">
      <div className="mx-auto w-full max-w-4xl px-4 pb-16 pt-10 sm:px-6">
        <header className="mb-6 flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-medium tracking-[0.22em] text-[#876a51]">ADMIN / RECIPE MANAGE</p>
        ...
        <section className="mb-5 rounded-2xl border border-[#e2d8cb] bg-[#f8f3ec] p-4">
          ...
          <form action={createRecipeAction} className="mt-3 grid gap-2">
            ...
            />
            <input name="garnish" placeholder="garnish" className="rounded-lg border border-[#d7cec2] bg-white px-3 py-2" />
            <textarea name="notes" placeholder="notes" className="min-h-16 rounded-lg border border-[#d7cec2] bg-white px-3 py-2" />
        ...
        {recipesError ? (
          ...
          <section className="space-y-3">
            {filteredRows.map((recipe) => (
              <form key={recipe.id} action={updateRecipeAction} className="rounded-2xl border border-[#e2d8cb] bg-[#f8f3ec] p-4">
              ...
              <div className="grid gap-2">
                ...
                <textarea
                  name="steps_input"
                  defaultValue={(recipe.recipe_steps ?? []).map((item) => item.instruction).join('\n')}
                  className="min-h-24 rounded-lg border border-[#d7cec2] bg-white px-3 py-2"
                />
                <input name="garnish" defaultValue={recipe.garnish ?? ''} className="rounded-lg border border-[#d7cec2] bg-white px-3 py-2" />
                <textarea name="notes" defaultValue={recipe.notes ?? ''} className="min-h-16 rounded-lg border border-[#d7cec2] bg-white px-3 py-2" />
              </div>
